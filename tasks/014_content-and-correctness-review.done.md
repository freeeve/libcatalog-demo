...
against what libcat and this repo actually do today, fix what's wrong, and add
...
- `README.md`: says schema version **5** (v6 since tasks/008); "Data" section documents
  ...
  cataloging sandbox (try.libcat.evefreeman.com), events/docs/CMS sections, or CI
...
- New docs guide covering the cataloging sandbox (what lcatd is, what to try at
  try.libcat, how nothing persists) -- the About page covers "what", docs lack the
  "how".
...
- `npm run build:full`; a11y audit `node ../libcat/hugo/a11y_audit.js public`;
...
try.libcat sandbox, CI deploy, current repo shape). build-and-deploy.md step 2 now
